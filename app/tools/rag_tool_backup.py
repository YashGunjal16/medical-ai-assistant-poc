...
class RAGManager:
    ...
    def __init__(self, persist_dir: str = "app/vectorstore/chroma"):
        ...
        try:
            ...
            # Initialize with reference materials if empty
    ...
    def _initialize_reference_materials(self):
        """Initialize default reference materials in vector store"""
        reference_docs = self._load_reference_materials()
        chunks = self._chunk_documents(reference_docs)
        
        try:
            self.vector_store.add_documents(chunks, source="default_reference")
            logger.info(f"Initialized vector store with {len(chunks)} reference chunks")
        except Exception as e:
...
"""
    ...
    def _chunk_documents(self, documents: List[Dict[str, str]], chunk_size: int = 600) -> List[tuple]:
        """
        Updated to return tuples compatible with VectorStore.add_documents format
    ...
    def query_reference_materials(self, query: str, n_results: int = 3) -> Dict[str, Any]:
        """
        Updated to use VectorStore search method with Gemini embeddings
            ...
            if search_results and search_results["documents"]:
                for doc, metadata in zip(search_results["documents"][0], search_results["metadatas"][0]):
                    formatted_results.append({
                        ...
                        "relevance_score": 1 - search_results["distances"][0][len(formatted_results)]  # Convert distance to similarity
                    })
    ...
    def add_pdf_documents(self, pdf_chunks: List[tuple], source: str = "pdf_upload") -> Dict[str, Any]:
        """
        New method to add PDF documents to vector store
    ...
    def get_vector_store_stats(self) -> Dict[str, Any]:
        """
        New method to expose vector store statistics
